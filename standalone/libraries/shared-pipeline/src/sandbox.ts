...
function runRepoTools(dir: string, files: any[]): any {
  return {
    ok: true,
    errors: [],
    lintErrors: [],
...
async function checkEditConstraints(dir: string, files: any[], options: any): Promise<any[]> {
  return [];
}

async function enforceMinimalDiffs(dir: string, files: any[]): Promise<any[]> {
  return [];
}

function inferAllowedPaths(spec: string, dir: string): string[] {
  return ['**/*'];
}
...
function generateConventionTestFile(files: any[], brief: any): string {
  return '// Convention tests placeholder';
}
