...
export function buildPromptWithContext(input: PromptInput): string {
  ...
  const sections = [
    ...
    '',
    '## OUTPUT FORMAT',
    'Return ONLY valid JSON. Support multi-file output for coordinated features:',
    '',
    ...
    '```json',
    '{',
    '  "files": [{"path": "src/example.ts", "content": "...full file content..."}],',
    '  "tests": [{"path": "src/__tests__/example.test.ts", "content": "...test content..."}]',
    '}',
    '```'
  ];
