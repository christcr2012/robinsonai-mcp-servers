 ...
 */
async function generateStepsFromSpec(specText: string, maxSteps: number): Promise<any[]> {
  // Try LLM first (if OLLAMA is reachable)
  const model = process.env.ARCHITECT_STD_MODEL || 'deepseek-coder:33b';
  try {
    const prompt = [
      "You are a senior software architect. Given a requirement/spec, produce a small JSON array of concrete,",
      "atomic implementation steps that a coding agent can execute. Favor deterministic actions and built-in",
      "tools already exposed in this system: file.patch_edit, npm.install_package, playwright.create_test,",
      "github.open_pr_with_changes. Each item must be {title, tool, params}.",
      "Limit to " + maxSteps + " steps. Respond with JSON only.",
      "",
      "SPEC:",
      specText
    ].join("\n");
  ...
  // Fallback: generic 5-step skeleton (no AI)
  return [
    { title: "Scaffold workspace", tool: "npm.install_package", params: { package: "vitest", dev: true } },
    { title: "Implement feature", tool: "file.patch_edit", params: { path: "src/index.ts", patch: "// TODO: implement" } },
    { title: "Create tests", tool: "playwright.create_test", params: { name: "basic.spec.ts", target: "src/index.ts" } },
...
async function startBackgroundPlanning(planId: number, budgets: PlanBudgets): Promise<void> {
  ...
  try {
    ...
    steps = await generateStepsFromSpec(specText, maxSteps);
