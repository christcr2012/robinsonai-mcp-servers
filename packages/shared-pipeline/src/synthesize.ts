...
function formatBriefForPrompt(brief: ProjectBrief): string {
  return JSON.stringify(brief, null, 2);
}

async function retrieveCodeContext(root: string, options: any): Promise<{ snippets: any[] }> {
  return { snippets: [] };
}
...
2. REPO-NATIVE CODE (HOUSE RULES)
   ...
   - Layering: ${brief.layering.layers ? brief.layering.layers.map((l: any) => `${l.name} can import ${l.canImport.join(', ')}`).join('; ') : 'respect folder boundaries'}
